...
struct TrayState {
    ...
    menu_webui_status: RefCell<nwg::MenuItem>,
    menu_webui_toggle: RefCell<nwg::MenuItem>,
    icon: nwg::Icon,
    ...
    webui_toggle_handle: nwg::ControlHandle,
}
    ...
    fn trigger_sync(&self, force: bool) {
            ...
            && !online_settings.auth_token.is_empty()
        {
            ...
            std::thread::spawn(move || {
                // No SyncGuard here — store-and-forward manages sync_in_progress itself
                // (unfreeze() in cleanup). Force-full / async / session modes no longer
                // apply; store-and-forward is a single path.
...
pub fn run(stop_signal: Arc<AtomicBool>, sync_state: Option<Arc<crate::lan_server::LanSyncState>>) -> TrayExitAction {
    ...
    // Kolejność pozycji utrzymywana w parytecie z macOS (src/platform/macos/tray.rs):
    // tytuł → Dashboard → blok sync → blok Web UI → Restart/Close.
    // NWG dodaje pozycje do menu w kolejności wywołań `.parent(&menu)`.
    ...
    let mut menu_sep_top = nwg::MenuSeparator::default();
    try_build!(
    ...
    let mut menu_sep_webui = nwg::MenuSeparator::default();
    try_build!(
        nwg::MenuSeparator::builder().parent(&menu),
        &mut menu_sep_webui,
        "webui separator"
    ...
    let mut menu_webui_status = nwg::MenuItem::default();
    try_build!(
        nwg::MenuItem::builder()
            .text(initial_lang.t(TrayText::WebUiStatusOff))
            .disabled(true)
            .parent(&menu),
        &mut menu_webui_status,
        "Web UI status menu item"
    );

    let mut menu_webui_toggle = nwg::MenuItem::default();
    try_build!(
        nwg::MenuItem::builder()
            .text(initial_lang.t(TrayText::WebUiStart))
            .parent(&menu),
        &mut menu_webui_toggle,
        "Web UI toggle menu item"
    );
    let webui_toggle_handle = menu_webui_toggle.handle;
    ...
    let state = Rc::new(TrayState {
        ...
        menu_webui_status: RefCell::new(menu_webui_status),
        menu_webui_toggle: RefCell::new(menu_webui_toggle),
        icon,
        icon_attention,
        icon_sync,
        sync_state,
