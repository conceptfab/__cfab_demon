...
struct AppliedTray {
    ...
    /// Czy blok sync (separator + status + 2 przyciski) jest obecnie w menu.
    /// Menu budowane jest z blokiem obecnym, więc start = Some(true).
    sync_block_visible: Option<bool>,
...
/// Wstawia lub usuwa blok sync (separator + status + 2 przyciski). Blok stoi
/// zawsze tuż po pozycji „Otwórz Dashboard" (indeks 2), więc na pozycjach 3..=6.
/// Wywoływać TYLKO przy zmianie stanu — podwójny insert zduplikowałby pozycje.
fn set_sync_block_visible(
    menu: &Menu,
    sync_separator: &PredefinedMenuItem,
    ...
    if visible {
        let _ = menu.insert(sync_separator, 3);
        let _ = menu.insert(sync_status_item, 4);
        let _ = menu.insert(sync_delta_item, 5);
        let _ = menu.insert(sync_force_item, 6);
    } else {
        let _ = menu.remove(sync_separator);
        let _ = menu.remove(sync_status_item);
        let _ = menu.remove(sync_delta_item);
        let _ = menu.remove(sync_force_item);
    }
}
...
fn update_sync_menu(
    ...
    if applied.sync_block_visible != Some(should_show) {
        ...
        applied.sync_block_visible = Some(should_show);
        // Po ponownym wstawieniu pozycje są „świeże" — wymuś re-aplikację
        // tekstu/stanu poniżej.
        applied.sync_status = None;
        applied.sync_actions_enabled = None;
    }
...
pub fn run(
    ...
    let webui_separator = PredefinedMenuItem::separator();
    ...
    // Separator należący do bloku sync — wstawiany/usuwany razem z pozycjami
    // sync, więc trzymamy go w nazwanej zmiennej (nie anonimowo).
    let sync_separator = PredefinedMenuItem::separator();

    let _ = menu.append(&version_item);
    let _ = menu.append(&PredefinedMenuItem::separator());
    let _ = menu.append(&dashboard_item);
    let _ = menu.append(&sync_separator);
    let _ = menu.append(&sync_status_item);
    let _ = menu.append(&sync_delta_item);
    let _ = menu.append(&sync_force_item);
    let _ = menu.append(&webui_separator);
    ...
    // Klon współdzieli wewnętrzny Rc menu — manipulacja przez `menu_handle`
    // (insert/remove bloku sync) działa na żywym menu tray.
...
fn launch_dashboard() {
    ...
    // 2) Szukanie .app obok daemona i w /Applications — wspólna lista z webui_host_ctl.
    let candidates = crate::webui_host_ctl::macos_app_candidates();
