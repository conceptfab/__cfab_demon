...
pub struct ProcessInfo {
    ...
    /// macOS: CFBundleIdentifier (lowercase) z NSRunningApplication.
...
pub struct ProcessSnapshot {
    ...
    /// (pid, pełna ścieżka exe lowercase) — do dopasowania po prefiksie bundle'a.
    pub pid_paths: Vec<(u32, String)>,
}
...
pub fn build_process_snapshot() -> ProcessSnapshot {
    ...
    let mut pid_paths: Vec<(u32, String)> = Vec::new();
    ...
    if let Some(entries) = crate::platform::process_snapshot::collect_process_entries() {
        for entry in entries {
            ...
            if let Some(path) = entry.exe_path {
                pid_paths.push((entry.process_id, path));
            }
            exe_pids
    ...
    ProcessSnapshot { tree, exe_pids, pid_paths }
}
...
/// Root-PIDy aplikacji: po dokładnej nazwie exe + (macOS) po prefiksie ścieżki bundle'a.
/// Prefiks porównujemy z separatorem ("/…/foo.app/"), żeby nie złapać "foo.app 2".
fn collect_app_pids(
    exe_name: &str,
    app_path: Option<&str>,
    proc_snap: &ProcessSnapshot,
) -> Vec<u32> {
    let mut root_pids = proc_snap
        .exe_pids
        .get(exe_name)
        .cloned()
        .unwrap_or_default();

    if let Some(prefix) = app_path {
        let prefix_dir = format!("{}/", prefix.trim_end_matches('/'));
        for (pid, path) in &proc_snap.pid_paths {
...
pub fn measure_cpu_for_app(
    ...
    let root_pids = collect_app_pids(exe_name, app_path, proc_snap);
...
mod tests {
    ...
    fn snapshot_with_paths() -> ProcessSnapshot {
        let mut exe_pids: HashMap<String, Vec<u32>> = HashMap::new();
        exe_pids.insert("antigravity".to_string(), vec![100]);
        ProcessSnapshot {
            tree: HashMap::new(),
            exe_pids,
            pid_paths: vec![
                ...
                (
                    300,
                    "/applications/antigravity ide 2.app/contents/macos/electron".to_string(),
                ),
                (100, "/applications/antigravity.app/contents/macos/antigravity".to_string()),
            ],
        }
    }

    #[test]
    fn collect_app_pids_matches_by_path_prefix() {
        let snap = snapshot_with_paths();
        let pids = collect_app_pids(
            "antigravity ide",
            Some("/applications/antigravity ide.app"),
            &snap,
        );
        // 200 + 201 z bundle'a; NIE 300 ("antigravity ide 2.app"), NIE 100 (inna appka)
        assert_eq!(pids, vec![200, 201]);
    }

    #[test]
    fn collect_app_pids_falls_back_to_exe_name_without_path() {
        let snap = snapshot_with_paths();
        assert_eq!(collect_app_pids("antigravity", None, &snap), vec![100]);
        assert!(collect_app_pids("antigravity ide", None, &snap).is_empty());
    }
