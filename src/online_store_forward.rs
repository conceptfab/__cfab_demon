...
fn execute_store_forward(
    ...
    // DOMYŚLNIE: dane lądują na storage-backendzie FTP/SFTP (np. „hostido"), a serwer
    // tylko koordynuje — zgodnie z architekturą produktu (dane NIGDY do bazy serwera).
    // Klucz do odczytu creds = klucz grupy (encryption_key, auto-wyprowadzany), więc
    // user nie wkleja żadnego sekretu. Stara ścieżka direct-sync→Postgres dostępna
    // wyłącznie jawnie przez sync_mode="direct-postgres" (legacy, normalnie nieużywane).
    if settings.sync_mode != "direct-postgres" {
        if settings.group_id.is_empty() {
            return Err("online sync FTP wymaga aktywnej grupy licencji (group_id) — aktywuj licencję".into());
        }
        // Najpierw drenaż pending (serwer blokuje push, gdy są zaległe paczki),
        // potem push tylko gdy lokalnie „dirty".
    ...
    // ── Legacy: direct-sync (snapshot → Postgres serwera), tylko gdy jawnie wybrane ──
