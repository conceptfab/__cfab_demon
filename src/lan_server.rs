...
pub struct LanSyncState {
    ...
    /// Whether at least one non-stale LAN peer is currently discovered.
...
fn handle_connection(
    ...
    if requires_auth {
        ...
        if expected.is_empty() || !constant_time_eq(&auth_secret, &expected) {
...
/// NOTE: ordered by local wall-clock `created_at` — clock skew between peers
/// can mis-pick the delta cutoff; tolerated because full sync converges.
/// See get_latest_marker in lan_sync_orchestrator.rs for the full rationale.
...
fn handle_pull(state: &LanSyncState, body: &str) -> (u16, String) {
    // Pull is only valid mid-session: the 13-step master freezes this slave
    // (step 5, /lan/freeze-ack) before pulling (step 6). Outside that window
    // a bare secret holder must not be able to exfiltrate the whole DB.
    if !state.db_frozen.load(Ordering::SeqCst) {
        sync_log("[SLAVE] /lan/pull odrzucony — brak aktywnej sesji sync (baza nie jest zamrozona)");
        return (409, json_error("no_active_sync"));
    }
    #[derive(Deserialize)]
    struct PullRequest {
        #[allow(dead_code)]
...
fn handle_trigger_sync(
    ...
    // Trigger endpoints are exempt from secret auth (the local bridge has no
    // secret) — so they MUST be loopback-only. A LAN host that could trigger
    // a sync with peer_ip pointing at itself would receive the paired peer's
    // secret in the preflight request headers.
    if !is_loopback(client_ip) {
        log::warn!("LAN trigger-sync rejected from non-loopback {}", client_ip);
...
fn handle_online_cancel_sync(state: &Arc<LanSyncState>, client_ip: IpAddr) -> (u16, String) {
    if !is_loopback(client_ip) {
        log::warn!("Online cancel-sync rejected from non-loopback {}", client_ip);
        return (403, json_error("loopback_only"));
    }
...
/// Constant-time string comparison for the LAN auth secret. The naive `!=`
/// short-circuits on the first differing byte, leaking secret prefixes through
/// response timing. XOR-fold touches every byte of the shorter input and folds
/// the length difference in, so timing is independent of where strings differ.
...
mod tests {
    ...
    fn pull_rejected_without_active_sync() {
        // /lan/pull exports the whole DB to any secret holder. In the 13-step
        // flow the master always freezes the slave (step 5) before pulling
        // (step 6), so an unfrozen state means there is no active sync session
        // and the pull must be refused.
        let state = LanSyncState::new();
        let body = serde_json::json!({
            "device_id": "some-master",
    ...
    fn constant_time_eq_matches_equal_strings() {
        assert!(constant_time_eq("secret-abc-123", "secret-abc-123"));
        assert!(constant_time_eq("", ""));
    }

    #[test]
    fn constant_time_eq_rejects_different_strings() {
        assert!(!constant_time_eq("secret-abc-123", "secret-abc-124"));
        assert!(!constant_time_eq("secret", "secret-longer"));
        assert!(!constant_time_eq("a", ""));
    }

    #[test]
    fn trigger_sync_rejects_non_loopback() {
        // Regression: /lan/trigger-sync was exempt from secret auth AND had no
        // loopback guard, so any LAN host could make the daemon send the paired
        // peer's secret (preflight X-TimeFlow-Secret header) to an attacker IP.
        let state = Arc::new(LanSyncState::new());
        let stop = Arc::new(AtomicBool::new(false));
        let body = serde_json::json!({
            "peer_ip": "192.168.1.50",
            "peer_port": 47891,
            "peer_device_id": "attacker-controlled",
        })
        .to_string();
        let (status, resp) = handle_trigger_sync(
            &state, &stop, &body,
            IpAddr::V4(Ipv4Addr::new(192, 168, 1, 50)),
        );
        assert_eq!(status, 403, "non-loopback trigger-sync must be rejected");
        assert!(resp.contains("loopback_only"));
        assert!(
            !state.sync_in_progress.load(Ordering::SeqCst),
            "rejected trigger must not leave sync_in_progress set"
        );
    }

    #[test]
    fn online_trigger_sync_rejects_non_loopback() {
        let state = Arc::new(LanSyncState::new());
        let stop = Arc::new(AtomicBool::new(false));
    ...
    #[test]
    fn online_cancel_sync_rejects_non_loopback() {
        let state = Arc::new(LanSyncState::new());
        let (status, resp) = handle_online_cancel_sync(
            &state,
            IpAddr::V4(Ipv4Addr::new(10, 0, 0, 7)),
        );
        assert_eq!(status, 403);
