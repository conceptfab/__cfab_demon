...
/// Version gate for the 13-step protocol: peers must run the same TIMEFLOW
/// version. The dashboard bridge enforces this for UI-triggered syncs
/// (dashboard commands/lan_sync.rs), but auto-sync paths (discovery, tray)
/// reach execute_master_sync directly — this is their only gate.
fn version_compat_error(local: &str, peer: Option<&str>) -> Option<String> {
    let peer = peer.unwrap_or("unknown");
    if peer == local {
        None
    } else {
        Some(format!(
            "version_mismatch: local v{}, peer v{} — update both machines",
            local, peer
        ))
    }
}
...
fn execute_master_sync(
    ...
    match preflight_resp {
        Ok(resp_str) => {
            if let Ok(resp) = serde_json::from_str::<serde_json::Value>(&resp_str) {
                ...
                let peer_version = resp.get("version").and_then(|v| v.as_str());
                if let Some(err) = version_compat_error(crate::VERSION.trim(), peer_version) {
                    sync_log(&format!("[2/13] PRZERWANE — {}", err));
                    return Err(err);
                }
    ...
    sync_common::backup_database_typed(&conn, "lan").map_err(|e| { sync_log(&format!("[8/13] BLAD backup: {}", e)); e })?;
    ...
    sync_common::merge_incoming_data(&mut conn, &slave_data)
        .map_err(|e| {
            ...
            if let Err(re) = sync_common::restore_database_backup_typed(&mut conn, "lan") {
    ...
    let db_ready_resp = db_ready_resp.map_err(|e| {
        // CELOWO bez restore backupu: lokalny merge zostal zweryfikowany
        // (krok 10). Restore cofalby tez wszystkie zapisy wykonane rownolegle
        // podczas okna db-ready (do ~10 min) — to byla realna utrata danych.
        // Peer dosynchronizuje sie przy nastepnej probie.
        sync_log(&format!(
            "[12/13] BLAD — slave nie zakonczyl importu po {} probach: {}. Lokalne scalenie pozostaje (zweryfikowane); peer dosynchronizuje sie przy nastepnej probie.",
            db_ready_timeouts.len(),
            e
        ));
        e
    })?;
    ...
    if let Ok(resp) = serde_json::from_str::<serde_json::Value>(&db_ready_resp) {
        if resp.get("ok").and_then(|v| v.as_bool()) != Some(true) {
            ...
            sync_log(&format!(
                "[12/13] Slave zglosil blad importu: {}. Lokalne scalenie pozostaje (zweryfikowane); peer dosynchronizuje sie przy nastepnej probie.",
                err_msg
            ));
...
///
/// KNOWN LIMITATION (clock skew): markers are ordered by `created_at`, a
/// locally-generated wall-clock string. If peers' clocks diverge, the delta
/// cutoff (`since`) can be wrong and a delta sync may skip rows. This is
/// self-healing — merge is union-based and the next full sync converges —
/// so we accept it instead of introducing a logical clock into the protocol.
...
mod tests {
    ...
    fn version_match_is_ok() {
        assert!(version_compat_error("0.1.5704", Some("0.1.5704")).is_none());
        // dev builds on both sides (VERSION fallback) must also pass
        assert!(version_compat_error("dev", Some("dev")).is_none());
    }

    #[test]
    fn version_mismatch_is_rejected() {
        let err = version_compat_error("0.1.5704", Some("0.1.5700")).expect("must reject");
        assert!(err.contains("version_mismatch"));
        assert!(err.contains("0.1.5704") && err.contains("0.1.5700"));
    }
