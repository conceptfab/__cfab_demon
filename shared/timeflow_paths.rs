...
// Stare nazwy migrowane do `TIMEFLOW`. Uwaga: `TimeFlow` (mieszana wielkość) to
// poprzednia nazwa kanoniczna — na systemach plików case-sensitive (Linux,
// case-sensitive APFS) to OSOBNY katalog, więc musi tu być, by dane się
// przeniosły. Na case-insensitive (zwykły macOS/Windows) `base.exists()` trafia
// w istniejący katalog i migracja jest pomijana (zero ryzyka dla danych).
const LEGACY_DIR_NAMES: [&str; 4] = ["conceptfab", "CfabDemon", "TimeFlowDemon", "TimeFlow"];
...
/// nie istnieje i migracją ze starszych nazw (conceptfab, CfabDemon, TimeFlow…).
