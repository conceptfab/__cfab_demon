...
export function LanPeerNotification() {
  ...
  const { incompatPeer, localVersion, syncError, syncing, visiblePeer } = state;
  ...
  useEffect(() => {
    ...
    getDaemonRuntimeStatus()
      .then((s) => {
        if (cancelled) return;
        dispatch({
          type: 'set_local_version',
          localVersion: (s.dashboard_version ?? '').trim(),
        });
      })
      .catch(() => {
        if (cancelled) return;
        dispatch({ type: 'set_local_version', localVersion: '' });
      });
    return () => {
      cancelled = true;
    };
    ...
    const poll = async () => {
      try {
        ...
        if (incompat?.device_id !== incompatPeerRef.current?.device_id) {
          dispatch({ type: 'set_incompat_peer', incompatPeer: incompat });
        }
      } catch {
  ...
  const handleSync = useCallback(async (peer: LanPeer) => {
    ...
    dispatch({ type: 'set_sync_error', syncError: null });
    try {
      ...
      if (msg.includes('Ping failed') || msg.includes('connection') || msg.includes('refused')) {
        dispatch({
          type: 'set_sync_error',
          syncError: t('settings.lan_sync.error_peer_unreachable'),
        });
      } else {
        dispatch({ type: 'set_sync_error', syncError: msg });
      }
  ...
  const handleIncompatDismiss = useCallback(() => {
    ...
    dispatch({ type: 'set_incompat_peer', incompatPeer: null });
  }, [incompatPeer]);
