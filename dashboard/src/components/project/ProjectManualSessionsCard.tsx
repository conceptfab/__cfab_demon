        ...
        <div className="space-y-3">
          {sessions.map((session) => (
            <button
              type="button"
              key={session.id}
              aria-label={t('accessibility.edit_manual_session', { title: session.title })}
              className="flex w-full items-center justify-between rounded-lg border border-border/40 bg-secondary/20 p-3 text-left cursor-pointer hover:bg-secondary/30 transition-colors"
