...
function ScoreBreakdownCandidates({
  scoreBreakdownData,
  variant,
}: {
          ...
          <div className="flex border-r border-border/5 pr-2 items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              {isSplittable && onSplitClick && (
                <button
                  type="button"
                  className="ml-1 inline-flex size-9 shrink-0 items-center justify-center rounded text-amber-400 hover:text-amber-300 cursor-pointer md:size-4"
                  onClick={(e) => {
                    e.stopPropagation();
                    onSplitClick(s);
                  }}
          ...
          <div className="flex items-center justify-between min-w-0">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {!isManual && (
                <button
                  type="button"
                  className="inline-flex h-9 items-center rounded border border-border/50 bg-secondary/30 px-2 text-[10px] font-semibold text-foreground/80 md:hidden"
              ...
              <button
                type="button"
                className="flex size-9 shrink-0 transform-gpu cursor-pointer items-center justify-center rounded-[2px] text-destructive/40 !transition-none hover:bg-destructive/10 hover:text-destructive md:size-4 md:text-destructive/30"
                onClick={async (e) => {
                  e.stopPropagation();
                  try {
              ...
              >
                <Trash2 className="size-4 md:size-2.5" />
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mb-1.5 h-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {!isManual && (
            <button
              type="button"
              className="inline-flex h-9 items-center rounded-md border border-border/50 bg-secondary/30 px-2 text-[11px] font-semibold text-foreground/80 md:hidden"
              onClick={(e) => handleContextMenu(e, s)}
            >
              {t('sessions.menu.assign_to_project')}
            </button>
          ...
          <div className="flex items-center">
            <button
              type="button"
              className="flex size-9 shrink-0 transform-gpu cursor-pointer items-center justify-center rounded-sm text-destructive/40 !transition-none hover:bg-destructive/10 hover:text-destructive md:size-5"
              onClick={async (e) => {
                e.stopPropagation();
                try {
                  await deleteSession(s.id);
