...
type SessionsVirtualListLoadState = {
...
type SessionsVirtualListProps = {
  ...
  loadState: SessionsVirtualListLoadState;
...
export function SessionsVirtualList({
  ...
  const { hasMore, isEmpty, isLoading, showUnassignedBanner } = loadState;
                ...
                return (
                  <div className="mt-4 space-y-1 first:mt-0">
                    <button
                      ...
                      aria-label={t('accessibility.filter_by_project', { name: group.projectName })}
                      className="group/hdr flex w-full cursor-pointer items-center justify-between gap-4 px-2 py-1 text-left leading-none"
                      onClick={() =>
                        onSelectProjectFilter(
