...
export function SessionScoreBadge({
  ...
  if (!onToggle) {
    return <div className="flex min-h-8 items-center gap-1.5 rounded-sm px-1 py-0.5 md:min-h-0">{content}</div>;
  }

  return (
    <button type="button"
      className="flex min-h-8 items-center gap-1.5 rounded-sm px-1 py-0.5 cursor-pointer hover:bg-white/10 !transition-none transform-gpu md:min-h-0"
