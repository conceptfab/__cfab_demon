...
export const addMonitoredApp = (
  exeName: string,
  displayName: string,
  bundleId?: string,
  appPath?: string,
) =>
  invokeMutation<void>('add_monitored_app', {
    exeName,
    displayName,
    bundleId,
    appPath,
...
export const inspectDroppedApp = (path: string) =>
  invoke<DroppedAppInfo>('inspect_dropped_app', { path });
...
export const refreshMissingDays = () =>
  invokeMutation<BackfillResult>('refresh_missing_days', undefined, {
    notify: (result) => result.days_backfilled > 0,
  });
