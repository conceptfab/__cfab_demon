...
export function useSessionsData(params: {
  ...
  const [prevFetchDeps, setPrevFetchDeps] = useState({ reloadVersion, buildFetchParams });
  ...
  if (
    prevFetchDeps.reloadVersion !== reloadVersion
    || prevFetchDeps.buildFetchParams !== buildFetchParams
  ) {
    setPrevFetchDeps({ reloadVersion, buildFetchParams });
    setIsLoading(true);
    setError(null);
  }
  ...
  useEffect(() => {
    let cancelled = false;
    isLoadingRef.current = true;
    // NIE zerujemy tu sessionsRef: ref musi odzwierciedlać wyrenderowany stan.
    // Wyzerowanie refa bez setSessions sprawiało, że gdy nowy zakres zwracał
    // pustą listę, replaceSessionsPage porównywał [] z [] i pomijał update —
    // na ekranie wisiały sesje z poprzedniego, niepustego zakresu dat.
