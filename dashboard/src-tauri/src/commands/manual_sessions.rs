...
fn parse_session_datetimes(start: &str, end: &str) -> Result<(i64, String), String> {
    let start_dt = chrono::NaiveDateTime::parse_from_str(start, "%Y-%m-%dT%H:%M:%S")
        .or_else(|_| chrono::NaiveDateTime::parse_from_str(start, "%Y-%m-%dT%H:%M"))
        .map_err(|e| format!("Invalid start_time: {}", e))?;
    let end_dt = chrono::NaiveDateTime::parse_from_str(end, "%Y-%m-%dT%H:%M:%S")
        .or_else(|_| chrono::NaiveDateTime::parse_from_str(end, "%Y-%m-%dT%H:%M"))
...
#[tauri::command]
pub async fn update_manual_session(
    app: AppHandle,
    id: i64,
    input: CreateManualSessionInput,
) -> Result<(), String> {
