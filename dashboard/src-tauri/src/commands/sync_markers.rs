...
#[tauri::command]
pub fn get_latest_sync_marker(app: AppHandle) -> Result<Option<SyncMarker>, String> {
    let conn = db::get_connection(&app)?;
...
#[tauri::command]
pub fn backup_before_sync(app: AppHandle) -> Result<String, String> {
    let conn = db::get_connection(&app)?;

    // Get DB path to determine backup directory
...
#[tauri::command]
pub fn markers_match(app: AppHandle, remote_marker_hash: Option<String>) -> Result<bool, String> {
    let local = get_latest_sync_marker(app)?;
