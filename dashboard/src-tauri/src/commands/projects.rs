...
///
/// For merged stages the parent's id is returned (not the stage's id), as long
/// as the parent is active (not excluded and not frozen). A frozen or excluded
/// parent yields `None` — the hint is dropped rather than sent to an inactive project.
pub(crate) fn ensure_app_project_from_file_hint(
    ...
    for candidate_name in candidates {
        let proj_id: Option<i64> = match conn.query_row(
            // Two-branch WHERE:
            // Branch 1 — active non-merged project: matches itself (returns p.id via COALESCE).
            // Branch 2 — merged stage: resolves to the ACTIVE parent (parent.id IS NOT NULL
            //   ensures the parent exists and is neither excluded nor frozen; a frozen/excluded
            //   parent yields no match so the hint is silently dropped).
            "SELECT COALESCE(parent.id, p.id)
             FROM projects p
             LEFT JOIN projects parent
               ON lower(parent.name) = lower(p.merged_into)
              AND parent.excluded_at IS NULL
              AND parent.frozen_at IS NULL
             WHERE lower(p.name) = lower(?1)
               AND (
                    (p.excluded_at IS NULL AND p.frozen_at IS NULL AND p.merged_into IS NULL)
                 OR (p.merged_into IS NOT NULL AND parent.id IS NOT NULL)
               )",
...
pub(crate) enum ProjectListFilter {
    Active,
    Excluded,
    Merged,
}
...
fn query_projects_with_stats(
    ...
    let filter = match list_filter {
        // merged_into guard is defensive: merged ⇒ excluded is enforced by
        // merge/restore/sync paths, but a broken invariant here would render
        // a child next to its parent with rolled-up (double-counted) time.
        ProjectListFilter::Active => "p.excluded_at IS NULL AND p.merged_into IS NULL",
        ProjectListFilter::Excluded => "p.excluded_at IS NOT NULL AND p.merged_into IS NULL",
        ProjectListFilter::Merged => "p.merged_into IS NOT NULL",
    };

    // Rollup (children's seconds added into parents) only makes sense for the
    // Active list — Excluded/Merged lists show raw own-time, otherwise an
    // excluded parent would display rolled totals next to its own children.
    let rollup = list_filter == ProjectListFilter::Active;
    ...
    // Dzienne rozbicie all-time per projekt (do zaokrąglania per_day). Bucket = data
    // (hourly=false), liczone tym samym silnikiem co totale — buckety i tak powstają.
    let mut all_time_daily_by_id: std::collections::HashMap<i64, Vec<i64>> =
    ...
    if let Some(all_time_range) = query_activity_date_range(conn)? {
        ...
        let daily_by_key = daily_seconds_by_series(&buckets);
        for (series_key, seconds) in &totals {
            if let Some(pid) = meta.get(series_key).and_then(|m| m.project_id) {
                all_time_totals.insert(pid, seconds.round() as i64);
                if let Some(daily) = daily_by_key.get(series_key) {
                    all_time_daily_by_id.insert(pid, daily.clone());
                }
            }
        }
    }
    ...
    if let Some(range) = date_range {
        period_totals = compute_project_clock_totals_by_id(conn, range, false, rollup)?
            .into_iter()
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            Ok(ProjectWithStats {
                ...
                frozen_at: row.get(9)?,
                merged_into: row.get(10)?,
                merged_at: row.get(11)?,
                daily_seconds: all_time_daily_by_id
                    .get(&project_id)
                    .cloned()
                    .unwrap_or_default(),
            })
    ...
    match list_filter {
        ...
        ProjectListFilter::Merged => {
            out.sort_by(|a, b| {
                b.merged_at
                    .as_deref()
                    .unwrap_or("")
                    .cmp(a.merged_at.as_deref().unwrap_or(""))
            });
        }
...
pub(crate) fn query_active_project_with_stats(
        ...
        query_activity_date_range(conn)?
    {
        ...
        match meta
            .iter()
            .find(|(_, m)| m.project_id == Some(id))
            .map(|(k, _)| k.clone())
        {
            Some(key) => {
                let total = totals.get(&key).copied().unwrap_or(0.0).round() as i64;
                let daily = daily_seconds_by_series(&buckets)
                    .remove(&key)
                    .unwrap_or_default();
                (total, daily)
            }
...
/// marker + auto-exclude + re-point apps. No session rows are touched —
/// time rolls up at query level (see rolled totals helpers).
pub(crate) fn merge_project_in_conn(
    conn: &mut rusqlite::Connection,
    source_id: i64,
    target_id: i64,
) -> Result<(), String> {
    ...
    let target_name: String = tx
        .query_row(
            "SELECT name FROM projects
             WHERE id = ?1 AND excluded_at IS NULL AND frozen_at IS NULL AND merged_into IS NULL",
            [target_id],
            |row| row.get(0),
        )
        .map_err(|_| "Target project must exist and be active".to_string())?;

    let source_name: String = tx
        .query_row(
            "SELECT name FROM projects WHERE id = ?1 AND merged_into IS NULL",
            [source_id],
            |row| row.get(0),
        )
        .map_err(|_| "Source project must exist and not be merged already".to_string())?;

    // Flatten: children of source become children of target (single-level hierarchy).
    tx.execute(
        "UPDATE projects SET merged_into = ?1, updated_at = datetime('now') WHERE merged_into = ?2",
        rusqlite::params![target_name, source_name],
    )
    .map_err(|e| e.to_string())?;

    // Future time goes to the parent (Layer 1 inheritance).
    tx.execute(
        "UPDATE applications SET project_id = ?1 WHERE project_id = ?2",
    ...
    tx.execute(
        "UPDATE projects
         SET merged_into = ?1,
             ...
             excluded_at = COALESCE(excluded_at, datetime('now')),
             updated_at = datetime('now')
         WHERE id = ?2",
        rusqlite::params![target_name, source_id],
    )
    .map_err(|e| e.to_string())?;
...
pub(crate) fn unmerge_project_in_conn(
    conn: &mut rusqlite::Connection,
    source_id: i64,
) -> Result<(), String> {
    let tx = conn.transaction().map_err(|e| e.to_string())?;
    let source_name: String = tx
        .query_row(
            "SELECT name FROM projects WHERE id = ?1 AND merged_into IS NOT NULL",
            [source_id],
            |row| row.get(0),
        )
        .map_err(|_| "Project is not merged".to_string())?;
    // Merge auto-excluded the project, which blacklisted its name
    // (trg_projects_blacklist_sync_exclude). Clear the entry first or
    // trg_projects_blacklist_block_update aborts the un-exclusion —
    // same order as restore_project.
    if let Some(name_key) = normalized_project_name_key(&source_name) {
        tx.execute(
...
pub(crate) fn restore_project_in_conn(
    ...
    // A merged project must stay excluded — clearing excluded_at would make it
    // collect time again and double-count after rollup. Unmerge handles both.
    let is_merged: bool = conn
        .query_row(
            "SELECT merged_into IS NOT NULL FROM projects WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .optional()
        ...
        .unwrap_or(false);
    if is_merged {
        return Err("Project is merged — unmerge it instead".to_string());
    }
...
pub(crate) fn delete_project_in_conn(
    ...
    let children: i64 = tx
        .query_row(
            "SELECT COUNT(*) FROM projects
             WHERE merged_into = (SELECT name FROM projects WHERE id = ?1)",
            [id],
            |row| row.get(0),
        )
        .map_err(|e| e.to_string())?;
    if children > 0 {
        return Err("Project has merged projects attached — unmerge them first".to_string());
    }
    tx.execute(
        "UPDATE applications SET project_id = NULL WHERE project_id = ?1",
        [id],
    ...
    .map_err(|e| e.to_string())?;
    // sessions.project_id was added via ALTER (m07) so it carries no FK — null it
    // explicitly to avoid a dangling reference to the deleted project. file_activities
    // and session_project_cache are handled by their ON DELETE SET NULL constraints.
    tx.execute(
...
pub(crate) fn delete_all_excluded_projects_in_conn(
    ...
    let ids: Vec<i64> = {
        let mut stmt = conn
            .prepare_cached(
                "SELECT id FROM projects
                 WHERE excluded_at IS NOT NULL AND merged_into IS NULL
                 AND name NOT IN (SELECT merged_into FROM projects WHERE merged_into IS NOT NULL)",
            )
            .map_err(|e| e.to_string())?;
        let rows = stmt
    ...
    for id in &ids {
        ...
        .map_err(|e| e.to_string())?;
        // See delete_project_in_conn: sessions.project_id has no FK, null it explicitly.
        tx.execute(
            "UPDATE sessions SET project_id = NULL WHERE project_id = ?1",
            [id],
        )
...
/// clock totals (children fold into the parent everywhere else too).
const PROJECT_WITH_MERGED_CHILDREN_FILTER: &str = "sp.project_id IN (
    SELECT id FROM projects
    WHERE id = ?3 OR merged_into = (SELECT name FROM projects WHERE id = ?3))";
...
pub(crate) fn query_project_extra_info(
        ...
        |conn: &rusqlite::Connection, start: &str, end: &str, p_id: i64| -> Result<f64, String> {
            let sql = format!(
                "{SESSION_PROJECT_CTE}
                 ...
                 WHERE {PROJECT_WITH_MERGED_CHILDREN_FILTER}"
            );
        ...
        all_time_bounds
    {
        ...
        let comment_and_boost_sql = format!(
                "{SESSION_PROJECT_CTE}
                 SELECT
                     COUNT(*) FILTER (WHERE {PROJECT_WITH_MERGED_CHILDREN_FILTER} AND sp.comment IS NOT NULL AND sp.comment <> ''),
                     COUNT(*) FILTER (WHERE {PROJECT_WITH_MERGED_CHILDREN_FILTER} AND sp.safe_rate_multiplier > 1.000001)
                 FROM session_projects sp"
            );
    ...
    let top_apps = if let Some((start, end)) = all_time_bounds {
        ...
        let rows = stmt
            .query_map(rusqlite::params![start, end, id], |row| {
                Ok(TopApp {
                    ...
                    seconds: row.get(1)?,
                    color: row.get(2)?,
                    daily_seconds: Vec::new(), // wypełniane niżej (przeskalowane per dzień)
                })
            })
            .map_err(|e| e.to_string())?;
        let mut apps = rows
        ...
        // Per-app seconds above are raw sums of duration_seconds; concurrently
        // running apps overlap and add up to more than the project's clock
        // total. distribute_app_seconds (the single home for per-app time math)
        // scales them down so the breakdown sums to the SAME clock total used
        // by the card, estimates, projects list, and report. Denominator is the
        // raw sum over ALL apps (not just the top 15) so hidden apps keep their
        // share.
        let raw_sum_all: f64 = conn
            .query_row(
                &format!(
                    "{SESSION_PROJECT_CTE}
                     SELECT COALESCE(SUM(CAST(sp.duration_seconds AS INTEGER)), 0)
                     FROM session_projects sp
                     WHERE {PROJECT_WITH_MERGED_CHILDREN_FILTER}"
                ),
                rusqlite::params![start, end, id],
        ...
        // distribute_app_seconds (gdy surowa suma apek > clock_total ⇒ skalujemy w dół),
        // żeby sumy dzienne były spójne z app.seconds. Pod per_day każdy dzień → pełna h.
        let factor = if raw_sum_all > 0.0 && clock_total > 0.0 && clock_total < raw_sum_all {
            clock_total / raw_sum_all
        } else {
            1.0
        };
        let daily_sql = format!(
            "{SESSION_PROJECT_CTE}
             ...
             LEFT JOIN applications a ON a.id = sp.app_id
             WHERE {PROJECT_WITH_MERGED_CHILDREN_FILTER}
             GROUP BY COALESCE(a.display_name, 'Unknown App'), sp.session_date"
        );
        let mut daily_stmt = conn.prepare(&daily_sql).map_err(|e| e.to_string())?;
        let daily_rows = daily_stmt
            .query_map(rusqlite::params![start, end, id], |row| {
                Ok((row.get::<_, String>(0)?, row.get::<_, i64>(1)?))
            })
            .map_err(|e| e.to_string())?;
        let mut daily_by_app: std::collections::HashMap<String, Vec<i64>> =
            std::collections::HashMap::new();
        for row in daily_rows {
            let (name, raw) = row.map_err(|e| format!("Failed to read daily app row: {}", e))?;
            let scaled = (raw as f64 * factor).round() as i64;
            if scaled > 0 {
...
mod tests {
    ...
    /// `initialize_database_file`, so triggers like m23 rename-cascade exist.
    ...
    #[test]
    fn merge_project_sets_marker_excludes_and_repoints_apps() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'Proj_final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'Proj_stage1', datetime('now'));
             INSERT INTO applications (id, executable_name, display_name, project_id)
             VALUES (10, 'app.exe', 'app', 2);",
        )
        .unwrap();
        ...
        let app_project: Option<i64> = conn
            ...
            .unwrap();
        assert_eq!(app_project, Some(1), "apps must be re-pointed to parent");
    }

    #[test]
    fn merge_project_rejects_inactive_target_and_self() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'A', datetime('now'));
    ...
    #[test]
    fn merge_project_flattens_hierarchy() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage2', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (3, 'stage1', datetime('now'));",
        )
        .unwrap();
    ...
    #[test]
    fn merged_projects_are_not_listed_as_excluded() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));
             INSERT INTO projects (id, name, created_at, excluded_at) VALUES (3, 'old', datetime('now'), datetime('now'));",
        )
        .unwrap();
        merge_project_in_conn(&mut conn, 2, 1).unwrap();
        ...
        let merged = query_projects_with_stats(&conn, ProjectListFilter::Merged, None).unwrap();
    ...
    #[test]
    fn excluded_and_merged_lists_show_raw_own_time_no_rollup() {
        // Conditional rollup on real time data: parent 'final' owns 3600s,
        // merged child 'stage1' owns 1800s (manual sessions keep the child's
        // project_id after merge — only `applications` are re-pointed).
        // Active list rolls child time into the parent (5400); once the
        // parent is ALSO excluded, the Excluded list must show its raw own
        // time (3600, not 5400) and the Merged list the child's raw 1800.
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));
             INSERT INTO manual_sessions (title, session_type, project_id, start_time, end_time, duration_seconds, date)
             VALUES ('parent work', 'work', 1, '2026-01-05T10:00:00', '2026-01-05T11:00:00', 3600, '2026-01-05');
             INSERT INTO manual_sessions (title, session_type, project_id, start_time, end_time, duration_seconds, date)
             VALUES ('child work', 'work', 2, '2026-01-05T12:00:00', '2026-01-05T12:30:00', 1800, '2026-01-05');",
        )
        .unwrap();
        merge_project_in_conn(&mut conn, 2, 1).unwrap();

        // Parent still active: Active list shows the rolled total (3600+1800).
        ...
        assert_eq!(
            excluded[0].total_seconds, 3600,
            "excluded parent shows raw own time, never the rolled 5400"
        );
        let merged = query_projects_with_stats(&conn, ProjectListFilter::Merged, None).unwrap();
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].name, "stage1");
    ...
    #[test]
    fn rename_parent_cascades_to_merged_children() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));",
        ...
        assert_eq!(
            m.as_deref(),
            Some("final_v2"),
            "m23 trigger must cascade rename"
        );
    }

    #[test]
    fn unmerge_project_restores_active_state() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));",
        ...
        .unwrap();
        merge_project_in_conn(&mut conn, 2, 1).unwrap();
        unmerge_project_in_conn(&mut conn, 2).unwrap();
        let (merged_into, excluded_at): (Option<String>, Option<String>) = conn
            .query_row(
                "SELECT merged_into, excluded_at FROM projects WHERE id = 2",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
        assert!(merged_into.is_none());
        assert!(excluded_at.is_none());
    }

    #[test]
    fn delete_project_blocked_when_has_merged_children() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));",
        ...
        .unwrap();
        merge_project_in_conn(&mut conn, 2, 1).unwrap();
        assert!(delete_project_in_conn(&mut conn, 1).is_err());
        unmerge_project_in_conn(&mut conn, 2).unwrap();
        assert!(delete_project_in_conn(&mut conn, 1).is_ok());
    }

    #[test]
    fn delete_project_nulls_session_project_id() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'Doomed', datetime('now'));
             INSERT INTO applications (id, executable_name, display_name, project_id)
             VALUES (10, 'app.exe', 'app', 1);
             INSERT INTO sessions (id, app_id, project_id, start_time, end_time, duration_seconds, date)
             VALUES (100, 10, 1, '2026-06-01 10:00:00', '2026-06-01 11:00:00', 3600, '2026-06-01');",
        )
        .unwrap();

        delete_project_in_conn(&mut conn, 1).unwrap();

        // No dangling reference: the session must point at no project, not at id 1.
        let session_pid: Option<i64> = conn
            .query_row("SELECT project_id FROM sessions WHERE id = 100", [], |r| {
                r.get(0)
            })
            .unwrap();
    ...
    #[test]
    fn delete_all_excluded_skips_merged_projects_and_their_parents() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));
        ...
        .unwrap();
        merge_project_in_conn(&mut conn, 2, 1).unwrap();
        // exclude the parent too — bulk cleanup must skip it because it has a merged child
        conn.execute(
            "UPDATE projects SET excluded_at = datetime('now') WHERE id = 1",
            [],
        )
        ...
        let deleted = delete_all_excluded_projects_in_conn(&mut conn).unwrap();
        assert_eq!(deleted, 1, "only 'old' must be deleted");
        let remaining: i64 = conn
            .query_row("SELECT COUNT(*) FROM projects", [], |r| r.get(0))
            .unwrap();
        assert_eq!(remaining, 2, "parent and merged child must survive");
    }

    #[test]
    fn restore_project_rejects_merged_child() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));",
        ...
        .unwrap();
        merge_project_in_conn(&mut conn, 2, 1).unwrap();
        assert!(restore_project_in_conn(&mut conn, 2).is_err());
    }
    ...
    #[test]
    fn ensure_app_project_from_file_hint_resolves_merged_to_parent() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'Proj_final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'Proj_stage1', datetime('now'));",
    ...
    #[test]
    fn clock_totals_rollup_param_folds_child_into_parent() {
        // Source-level fold: with rollup_merged=true the child's series is
        // folded into the parent (parent = own + child, child key gone); with
        // false both projects keep their raw own-time.
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'stage1', datetime('now'));
        ...
        assert!(
            ...
            "child series must fold away with rollup_merged=true"
        );

        let raw = super::compute_project_clock_totals_by_id(&conn, &range, false, false).unwrap();
        assert_eq!(raw.get(&1).copied().map(f64::round), Some(3600.0));
        assert_eq!(raw.get(&2).copied().map(f64::round), Some(1800.0));
    }

    #[test]
    fn ensure_app_project_from_file_hint_skips_merged_with_frozen_parent() {
        let mut conn = test_conn();
        conn.execute_batch(
            "INSERT INTO projects (id, name, created_at) VALUES (1, 'Proj_final', datetime('now'));
             INSERT INTO projects (id, name, created_at) VALUES (2, 'Proj_stage1', datetime('now'));",
