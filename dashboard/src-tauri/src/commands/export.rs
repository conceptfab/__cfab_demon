...
fn build_export_archive(
    ...
    let (archive, default_name) = {
            ...
            let mut stmt = conn
                .prepare(
                    "SELECT s.id, s.app_id, s.project_id, s.start_time, s.end_time, s.duration_seconds, s.date
                        ...
                        , COALESCE(s.project_name, (SELECT p.name FROM projects p WHERE p.id = s.project_id))
                 FROM sessions s
        ...
        // 5.3 Fetch File Activities (szczegóły plików/tytułów okien — dane AI
        // i widok Detailed). Scoping jak przy sesjach: temp table _export_app_ids
        // (pełny eksport = wszystkie aplikacje, eksport projektu = aplikacje projektu).
        let mut file_activities = Vec::new();
        if !app_ids.is_empty() {
            let mut stmt = conn
                .prepare(
                    "SELECT fa.app_id, fa.date, fa.file_name, fa.file_path, fa.total_seconds,
                            fa.first_seen, fa.last_seen, fa.project_id, fa.window_title,
                            fa.detected_path, fa.title_history, fa.activity_type, fa.activity_spans
                     FROM file_activities fa
                     INNER JOIN _export_app_ids e ON e.id = fa.app_id
                     WHERE fa.date >= ?1 AND fa.date <= ?2
                     ORDER BY fa.app_id, fa.date, fa.file_path",
                )
                .map_err(|e| e.to_string())?;
            let fa_rows = stmt
                .query_map([&start, &end], |row| {
                    Ok(FileActivityExportRow {
                        app_id: row.get(0)?,
                        date: row.get(1)?,
                        file_name: row.get(2)?,
                        file_path: row.get(3)?,
                        ...
                        detected_path: row.get(9)?,
                        title_history: row.get(10)?,
                        activity_type: row.get(11)?,
                        activity_spans: row.get(12)?,
                    })
                })
                .map_err(|e| e.to_string())?;

            for fa in fa_rows {
                file_activities.push(fa.map_err(|e| e.to_string())?);
            }
        }
