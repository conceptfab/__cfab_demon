...
pub fn retrain_model_sync(conn: &mut rusqlite::Connection) -> Result<i64, String> {
    ...
    let result = (|| -> rusqlite::Result<i64> {
            ...
            "INSERT INTO assignment_model_app (app_id, project_id, cnt, last_seen)
             ...
             WHERE s.project_id IS NOT NULL
               ...
               AND COALESCE((
                     ...
                     LIMIT 1
                   ), '') NOT IN ('auto_accept', 'deterministic_rule')
        ...
        {
            // Label comes from the SESSION (with the same user-only feedback filter
            // as the app/time layers), not from file_activities.project_id which may
            // have been written by an auto-safe run.
            let mut file_stmt = tx.prepare(
                "SELECT fa.app_id, fa.file_name, fa.file_path, fa.detected_path, s.project_id,
                        fa.window_title, fa.title_history, fa.date
                 FROM sessions s
            ...
            while let Some(row) = file_rows.next()? {
                ...
                if let Some(ref wt) = window_title {
                    let cleaned = timeflow_shared::title_parser::extract_file_from_title(wt);
                    for token in tokenize(&cleaned) {
                        *token_counts.entry((token, project_id)).or_insert(0.0) += weight;
                    }
                }
                for title in parse_title_history(title_history.as_deref()) {
                    let cleaned = timeflow_shared::title_parser::extract_file_from_title(&title);
                    for token in tokenize(&cleaned) {
                        *token_counts.entry((token, project_id)).or_insert(0.0) += weight;
                    }
                }
...
mod tests {
    ...
    fn retrain_model_supports_split_feedback_rows_without_explicit_weight() {
        ...
        // With decay and duration weighting, the matching session contributes ~2,
        // plus feedback boost of 3 (1.0 * 3.0 rounded) → ~5
        assert!(app_cnt >= 4 && app_cnt <= 7, "app_cnt was {}", app_cnt);
        assert!(time_cnt >= 4 && time_cnt <= 7, "time_cnt was {}", time_cnt);
    }

    #[test]
    fn retrain_skips_tokens_from_auto_accepted_sessions() {
        let mut conn = setup_training_conn();
        let (start, end, date) = session_window(1, 11);
        ...
        let auto_tokens: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM assignment_model_token WHERE token = 'secretalpha'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        let manual_tokens: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM assignment_model_token WHERE token = 'manualbeta' AND project_id = 20",
                [],
                |r| r.get(0),
            )
            .unwrap();
