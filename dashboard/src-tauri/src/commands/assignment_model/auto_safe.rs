...
/// Assigns file_activities in the session window to `project_id`,
/// but ONLY rows that have no assignment yet. Without the IS NULL guard an
/// auto-run would overwrite daemon/path/manual assignments and the scoring
/// layer would later read the model's own decision back as Layer-0 "fact".
pub(crate) fn assign_file_activities_for_window(
...
/// Rollback counterpart: restores `from_project_id` ONLY on rows that still
/// carry the run's `to_project_id` — rows assigned by other means are kept.
pub(crate) fn revert_file_activities_for_window(
    conn: &rusqlite::Connection,
...
pub fn rollback_sync(conn: &mut rusqlite::Connection) -> Result<AutoSafeRollbackResult, String> {
    ...
    for row in item_rows {
        ...
        if let Some((date, start_time, end_time)) = session_time {
            revert_file_activities_for_window(
                &trx,
                from_project_id,
                to_project_id,
                app_id,
...
mod tests {
    ...
    fn setup_fa_conn() -> rusqlite::Connection {
        ...
        conn.execute_batch(
            ...
            INSERT INTO file_activities (id, app_id, date, file_name, file_path, first_seen, last_seen, project_id) VALUES
                (1, 1, '2026-06-10', 'a.rs', '/p/a.rs', '2026-06-10T10:00:00+00:00', '2026-06-10T11:00:00+00:00', NULL),
                (2, 1, '2026-06-10', 'b.rs', '/p/b.rs', '2026-06-10T10:00:00+00:00', '2026-06-10T11:00:00+00:00', 77);",
        )
        .expect("schema");
        conn
    }
    ...
    #[test]
    fn assign_file_activities_skips_already_assigned_rows() {
        let conn = setup_fa_conn();
        super::assign_file_activities_for_window(
            &conn,
            99,
            1,
            "2026-06-10",
