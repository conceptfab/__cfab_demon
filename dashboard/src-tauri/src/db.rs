...
pub(crate) fn repair_ghost_project_names(conn: &rusqlite::Connection) -> Result<usize, String> {
    let fixed = conn
        .execute(
            "UPDATE sessions
             SET project_id = (
                 SELECT id FROM projects
                 WHERE name = sessions.project_name
        ...
        .map_err(|e| format!("repair_ghost_project_names failed: {}", e))?;

    if fixed > 0 {
        log::info!(
            "Startup repair: resolved project_id for {} session(s) with ghost project_name",
            fixed
        );
    }
...
mod tests {
    ...
    #[test]
    fn repair_ghost_project_names_resolves_matching_projects() {
        ...
        conn.execute_batch(
            "CREATE TABLE projects (
             ...
             INSERT INTO projects VALUES (1, 'notch', NULL);
             INSERT INTO projects VALUES (2, 'excluded_proj', '2026-01-01');
             INSERT INTO sessions VALUES (10, NULL, 'notch');
             INSERT INTO sessions VALUES (11, NULL, 'excluded_proj');
             INSERT INTO sessions VALUES (12, 1, 'notch');
             INSERT INTO sessions VALUES (13, NULL, NULL);
             INSERT INTO sessions VALUES (14, NULL, 'unknown_project');",
        )
        .expect("setup");

        let fixed = repair_ghost_project_names(&conn).expect("repair ok");
