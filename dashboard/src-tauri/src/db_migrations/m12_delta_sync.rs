...
pub fn run(tx: &Connection) -> Result<(), rusqlite::Error> {
    ...
    // Historical version (sync_key = app_id|start_time), superseded by m21 —
    // intentionally NOT unified with tombstone_triggers::SESSIONS_TOMBSTONE_TRIGGER_SQL.
    tx.execute(
    ...
    tx.execute(
        super::tombstone_triggers::APPLICATIONS_TOMBSTONE_TRIGGER_SQL,
        [],
    )?;
