    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            if headless {
                ...
                // Okno pozostaje ukryte (visible:false). Serwer startuje tylko gdy
                // Web Server jest włączony w ustawieniach — inaczej proces headless
                // nie ma czego serwować, więc kończymy go, by nie zostawiać sieroty.
                if !webui::start_headless(app.handle()) {
                    log::info!("[webui] headless exit: Web Server disabled in settings");
                    // process::exit nie emituje RunEvent::Exit, więc cleanup
                    // webui_host.json się tu nie wykona — i nie musi: serwer nie
                    // wstał, więc plik statusu nie został zapisany.
                    std::process::exit(0);
                }
            } else {
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::project_set_client,
            commands::project_set_status,
            commands::projects_with_client,
            commands::clients_sync_from_pm,
            ...
            commands::get_project_limit_status,
            commands::get_projects_limit_overview,
            commands::update_project_limit,
            commands::apply_project_limit_boost,
            ...
            commands::list_time_algorithms,
            commands::get_time_algorithm,
            commands::set_time_algorithm,
            ...
            commands::get_unassigned_cfab_renders,
            commands::get_all_cfab_renders,
            commands::assign_cfab_render,
            commands::reassign_cfab_render,
            commands::detach_cfab_render,
            commands::infer_dcc_session_project,
            commands::get_dcc_render_overlap,
            ...
            commands::reset_model_weights,
            commands::reset_model_full,
            ...
            commands::get_persisted_language,
