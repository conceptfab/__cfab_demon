...
- [ ] **63:** Helper `useClickOutsideDismiss(ref, onClose)` + `useEscapeKey(onClose)` w `dashboard/src/hooks/useDismissable.ts`; zastąp TODO w `Sessions.tsx:252,311`.

- [ ] **Commit per grupa (UI 59-60, UI 61, UI 62, UI 63)**
...
- [ ] **66:** `compare_locales.py` — ścieżki: `Path(__file__).resolve().parent / "dashboard/src/locales"`.
...
- [ ] **84:** `get_machine_name` — jedno miejsce (`lan_common.rs:85-87`), usuń duplikat z `lan_discovery.rs:128-130`.
- [ ] **85:** `execute_online_sync` + `execute_online_sync_inner` — scal w jedną funkcję z parametrem `force_full: bool`.
- [ ] **86:** `LEGACY_*` migracja `cfab_dashboard.db` → usuń po `2026-12-31` (scheduled). Dodaj TODO z datą w kodzie.
